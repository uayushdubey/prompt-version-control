...
def _resolve_repo(root: Optional[str] = None) -> PromptRepo:
    ...
    if not repo.storage.is_initialized:
        raise RuntimeError(
            ...
            "Run `promptvc init` or PromptRepo().init_repo() first."
        )
    return repo
...
class RunContext:
    ...
    Usage:
        with promptvc.run_context("summarizer", "v2", provider="openai") as ctx:
...
class PromptDecorator:
    ...
    Usage:
        @promptvc.prompt("summarizer", version="v2", provider="openai")
...
def prompt(
    ...
    Example:
        @promptvc.prompt("summarizer", version="v3", provider="openai")
        def summarize(text: str) -> RunResult: ...
    """
...
def run(
    ...
    **variables,
    ...
    Example:
        result = promptvc.run("summarizer", "v2", provider="openai", text="Hello world")
        print(result.output)
    """
    repo = _resolve_repo()
    kwargs: Dict[str, Any] = {}
...
def batch_run(
    ...
    Example:
        ...
        for r in results.results:
            print(r.output)
        print(f"Total cost: {promptvc.format_cost(results.total_cost_usd)}")
    """
    if inputs is None:
        inputs = [{}]
