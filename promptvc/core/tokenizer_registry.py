"""
promptvc/core/tokenizer_registry.py

Production-grade tokenizer registry with model-aware BPE tokenization.

Hierarchy:
  1. tiktoken (real BPE) — if installed: `pip install promptvc[tiktoken]`
  2. WordPunct (regex) — always available, ~10% off
  3. Character estimate (4 chars/token) — always available, rough

Usage:
    from promptvc.core.tokenizer_registry import get_tokenizer_for_model, TokenizerRegistry
...
class TokenizerRegistry:
    ...
    def get(name: str) -> BaseTokenizer:
        from promptvc.core.repo import get_tokenizer
