...
The master key is derived from one of three sources (in priority order):
  ...
  3. Auto-generated key stored in ~/.promptvc_master (user home, not project)

Keys are stored in `.promptvc/secrets.enc` as a JSON dict encrypted at rest.

CLI usage:
    promptvc secrets set openai sk-...
    promptvc secrets get openai
    promptvc secrets list
    promptvc secrets delete openai

Python usage:
    from promptvc.security.secrets import SecretsStore
    store = SecretsStore(root=Path(".promptvc"))
    store.set("openai", "sk-...")
...
def _get_master_key() -> bytes:
    ...
    Priority:
    1. OS keyring ('promptvc' service, 'master_key' username)
    2. PROMPTVC_MASTER_KEY environment variable
    3. Auto-generated key persisted in ~/.promptvc_master
    ...
    try:
        import keyring
        stored = keyring.get_password("promptvc", "master_key")
        if stored:
    ...
    key_path = Path.home() / ".promptvc_master"
    if key_path.exists():
        try:
...
def _get_fernet():
    ...
    try:
        ...
        raise SecretsError(
            ...
            "Install it with: pip install promptvc[secrets]"
        )


class SecretsStore:
    """
    Encrypted secrets store backed by .promptvc/secrets.enc.
