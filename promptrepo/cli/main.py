...
from promptrepo.utils.template import (
    render_template,
    extract_variables,
    find_unused_variables,
)
from promptrepo.utils.console import (
    safe_print,
    print_error_panel,
    success,
    error,
...
from promptrepo.utils.cost import (
    estimate_cost, format_cost, format_latency,
    compute_cost_breakdown, format_cost_breakdown, CostBreakdown,
)
from promptrepo.utils.diff import compute_diff, apply_unified_diff
...
def status_command(args: argparse.Namespace) -> None:
    ...
    print_box("promptrepo status", summary_lines)
...
def commit_command(args: argparse.Namespace) -> None:
    ...
    safe_print()
    safe_print(_colorize(f"  Run it: promptrepo run {name} {result['id']}", Color.CYAN))
    safe_print()
...
def inspect_command(args: argparse.Namespace) -> None:
    ...
    try:
        ...
        print_error_panel(
            ...
            hint_cmd=f"promptrepo log {args.name}",
        )
        return
    ...
    cmd = f"  promptrepo run {args.name} {args.version}"
    if var_flags:
        cmd += f" {var_flags}"
...
def list_command(args: argparse.Namespace) -> None:
    ...
    try:
        ...
        if not spaces:
            safe_print(dim("\n  No prompt spaces found. Run: promptrepo commit <name>"))
            return
...
def run_command(args: argparse.Namespace) -> None:
    ...
    try:
        ...
        print_error_panel(
            ...
            tips=[
                ...
                f"Set default with: promptrepo config set provider openai",
            ],
        )
        return
    ...
    try:
        ...
        print_error_panel(
            ...
            tips=[f"Run: promptrepo log {args.name}  to see available versions"],
        )
        return
    ...
    try:
        ...
        print_error_panel(
            ...
            hint_cmd=f"promptrepo inspect {args.name} {args.version}",
        )
        return
...
"""
    ...
    backup_path = file_path + ".promptrepo.bak"
...
def test_run_command(args: argparse.Namespace) -> None:
    ...
    safe_print()
    safe_print(bold(f"  promptrepo test  ·  {args.name} @ {args.version}" + (f" (comparing to {compare_version})" if compare_version else "")))
...
def test_command(args: argparse.Namespace) -> None:
    ...
    if sub == "run":
        ...
        print_error_panel(
            ...
            tips=["Usage: promptrepo test run <name> <version> --suite <file>",
                  "       promptrepo test golden <name> <version> --suite <file>",
                  "       promptrepo test list"],
        )
        sys.exit(1)
...
def _cmd_help() -> None:
    ...
    safe_print()
    print_table(["Command", "Description"], rows, title="promptrepo shell")
    safe_print()
...
def pipe_run_command(args: argparse.Namespace) -> None:
    ...
    try:
        ...
        print_error_panel(
            ...
            tips=["Ensure the file exists and is valid JSON",
                  "See: promptrepo pipe validate <file>"],
        )
        sys.exit(1)
...
def pipe_command(args: argparse.Namespace) -> None:
    ...
    if sub == "run":
        ...
        print_error_panel(
            ...
            tips=["Usage: promptrepo pipe run <pipeline.json> [--var key=value]",
                  "       promptrepo pipe validate <pipeline.json>"],
        )
        sys.exit(1)
...
def validate_command(args: argparse.Namespace) -> None:
    ...
    if sub == "dataset":
        if not getattr(args, "file", None):
            print_error_panel("Missing dataset file path.", tips=["Usage: promptrepo validate dataset <file>"])
            sys.exit(1)
        ...
        if not getattr(args, "name", None) or not getattr(args, "version", None):
            print_error_panel("Missing prompt name or version.", tips=["Usage: promptrepo validate prompt <name> <version>"])
            sys.exit(1)
        repo = PromptRepo()
        ...
        print_error_panel(
            ...
            tips=[
                "Usage: promptrepo validate dataset <file>",
                "       promptrepo validate prompt <name> <version>",
            ],
        )
        sys.exit(1)
...
def trace_command(args: argparse.Namespace) -> None:
    ...
    if not repo.storage.is_initialized:
        print_error_panel("Repository not initialized.", tips=["Run 'promptrepo init' first."])
        sys.exit(1)
...
def secrets_command(args: argparse.Namespace) -> None:
    ...
    if not store.is_available:
        print_error_panel(
            ...
            tips=["Install with: pip install promptrepo[secrets]"],
        )
        return
    ...
    if sub == "set":
        ...
        try:
            ...
            if key:
                ...
                safe_print(dim(f"  Hint: promptrepo secrets set {service} <key>"))
        except Exception as exc:
        ...
        try:
            ...
            if not services:
                ...
                safe_print(dim("  Hint: promptrepo secrets set openai <key>"))
            else:
                safe_print()
...
def import_command(args: argparse.Namespace) -> None:
    ...
    if "spaces" not in data:
        print_error_panel(
            ...
            tips=["Expected a file created by: promptrepo export"]
        )
        return
...
Examples:
  promptrepo init
  promptrepo commit summarize --message "v1" --prompt "Summarize: {{text}}"
  promptrepo run summarize v1 --provider openai
  promptrepo test run summarize v1 --suite tests/summarize.json
  promptrepo shell
  promptrepo pipe run pipeline.json --var code="$(cat src/main.py)"
  promptrepo status
""",
    )
    parser.add_argument("--version", action="version", version="promptrepo 0.1.0")
    parser.add_argument("--json", action="store_true", help="Output machine-readable JSON format")

    subparsers = parser.add_subparsers(dest="command", required=True)

    # init
    subparsers.add_parser("init", help="Initialize a promptrepo repository")
...
def main() -> None:
    ...
    try:
        ...
        print_error_panel(
            ...
            tips=["Run: promptrepo --help  to see available commands"],
        )
        sys.exit(1)
    except PromptRepoError as exc:
        print_error_panel(str(exc))
